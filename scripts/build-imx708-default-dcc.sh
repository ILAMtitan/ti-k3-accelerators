...
# 2304x1296 RAW10 mode. This is a bring-up profile, not production tuning.
#
# Compatibility detail: SENSOR_ID=219 is intentional for r1. The installed
# J722S tiovx sensor library does not yet expose IMX708, so the board smoke
# pipeline uses SENSOR_SONY_IMX219_RPI only as a numeric DCC registry slot.
# The actual DCC content generated here is for IMX708 geometry/RAW10 and AE/AWB
# are disabled on the board.
...
SENSOR_TREE=imx708_openhd_compat219
CONFIG_NAME=imx708_openhd_2304x1296_properties.txt
...
cp -f "$VISS" "$OUT/dcc_viss_2304x1296.bin"
cp -f "$A2" "$OUT/dcc_2a_2304x1296.bin"
...
bayer_pattern=RGGB
dcc_compat_id=219
ae_awb=disabled during r1 smoke test
imaging_ref=$IMAGING_REF
source_viss=$VISS
source_2a=$A2
META

sha256sum "$OUT/dcc_viss_2304x1296.bin" "$OUT/dcc_2a_2304x1296.bin" | tee "$OUT/SHA256SUMS"
echo
