...
[ "$#" -eq 1 ] && [ "$jobs" -gt 0 ] || usage
armbian="$(readlink -f "$1")"
[ -x "$armbian/compile.sh" ] || { echo "Missing Armbian compile.sh: $armbian" >&2; exit 1; }
[ -f "$armbian/userpatches/config-ti-k3-beagley-ai-waveshare.conf" ] || { echo 'Waveshare TI K3 userpatches are not prepared' >&2; exit 1; }
...
(
    ...
    ./compile.sh build ti-k3-beagley-ai-waveshare
)
